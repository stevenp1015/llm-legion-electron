...
const MinionsPanel: React.FC<MinionsPanelProps> = ({
  ...
  return (
    <>
      <div
        className={`fixed inset-0 bg-gradient-to-br from-zinc-400 via-zinc-700 to-zinc-900 z-10 transition-opacity duration-1000 ease-in-out ${
      ...
      <div className={`fixed top-0 right-0 h-full w-full max-w-sm bg-zinc-50/75 backdrop-blur-xl shadow-2xl z-40 transform transition-transform duration-500 ease-in-out border-l border-amber-500 rounded-tl-2xl rounded-bl-2xl ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex flex-col h-full">
          <div className="p-4 bg-white border-b border-zinc-200 rounded-tl-2xl flex justify-between items-center flex-shrink-0">
            <h2 className="text-xl font-semibold text-zinc-700">Roster & Tools</h2>
            <button onClick={onToggle} className="p-1 text-zinc-500 hover:text-neutral-900">
          ...
          <div className="flex-grow p-4 space-y-4 overflow-y-auto scrollbar-thin scrollbar-thumb-neutral-400 scrollbar-track-zinc-200">
            ...
            <div className="space-y-3">
              <h3 className="text-md font-semibold text-zinc-700">Minion Roster</h3>
              {minionConfigs.length === 0 && (
                <p className="text-sm text-zinc-500 text-center py-4">No Minions deployed. Add one to begin operations!</p>
              )}
              {[...minionConfigs].sort((a, b) => a.name.localeCompare(b.name, undefined, { sensitivity: 'base' })).map(config => (
                <div key={config.id} className={`p-1 bg-gradient-to-br border-r-2 border-l-4 border-t-4 border-b-2 from-zinc-50 from-50% to-zinc-200  rounded-lg shadow-ok 
                  ${config.role === 'regulator'
                    ? 'border-r-amber-700 border-b-amber-600 border-l-amber-400 border-t-amber-500' 
                    : 'border-r-teal-700 border-b-teal-600 border-l-teal-400 border-t-teal-500'}`}
                  >
                  
                  <div className="flex items-start justify-between">
                    <div className="flex items-center gap-2 overflow-hidden">
                      <MinionIcon name={config.name} className="w-8 h-8" />
                      <div className="overflow-hidden">
                        ...
                        <p className={`text-xs font-bold uppercase tracking-wider mt-1 text-amber-500`}>
                          {config.role === 'regulator' && (
                            <p>Regulator</p>
                          )}
                        </p>
                    ...
                    <div className="flex flex-row flex-shrink-0">
                      ...
                      <button
                        onClick={() => handleBulkRemovalOpen(config.name)}
                        className="p-1 text-neutral-500 hover:text-orange-500 transition-colors"
                        title={`Remove ${config.name} from channels`}
                        >
...
export default React.memo(MinionsPanel);
